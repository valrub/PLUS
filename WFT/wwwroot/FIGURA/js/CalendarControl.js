...
function CalendarControl()
{
    ...
    function getProperty(p_property)
    {
        ...
        if (elm != null)
        {
            if (elm.style) { elm = elm.style; if (elm[p_property]) { return elm[p_property]; } else { return null; } } else { return null; }
        }
    }
    ...
    function clearDate()
    {
        dateField.value = ''; dateField.focus(); hide();
    }
    this.setDate = setDate;

    function setDate(year, month, day)
    {
        if (dateField) { if (day < 10) { day = "0" + day; } if (month < 10) { month = "0" + month; } var dateString = month + "-" + year; dateField.value = dateString; dateField.focus(); dateField.value += ''; if (dateField.onchange != null) __doPostBack(dateField.id, ''); hide(); } return;
    }
    ...
    {
        currentYear += change; currentDay = 0;
        calendar = document.getElementById(calendarId);
        calendar.innerHTML = calendarDrawTable();
    }
    ...
    function hide()
    {
        if (dateField)
        {
            setProperty('display', 'none'); setElementProperty('display', 'none', 'CalendarControlIFrame'); dateField = null;
        }
    }
...
{
    if (!x) { x = 0 } if (!y) { y = 0 } var field = document.getElementById(textField); calendarControl.show(field, x, y);
}
