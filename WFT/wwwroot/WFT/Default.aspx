    ...
    <body>
		...
		<div id="reportReview">
			...
			<table id="badReportReview" style="width: 100%;">
				...
				<tr><td>Shares</td><td id="New_Shares"></td><td id="Missing_Shares"></td></tr>
			...
			$('td').click(function() {
				if (this.getAttribute('title')) {
					...
					if (this.getAttribute('class') == 'imageReport') {
						...
						$('#Shares').html($xml.find("Shares").text());
						...
						$('#reportReview,#imageReportReview').show();
						...
						$('#New_Shares').html($xml.find("New_Shares").text());
						$('#Missing_Likes').html($xml.find("Missing_Likes").text());
						$('#Missing_Shares').html($xml.find("Missing_Shares").text());
						...
						var $badReportRows = $('#badReportReview tr');
						var lastRowIndex = $badReportRows.length;
						var New_Total = 0, Missing_Total = 0;
						$.each($badReportRows, function(index){
							var $td1 = $(this).find('td:eq(1)');
							var $td2 = $(this).find('td:eq(2)');
							if (index>0 && index<lastRowIndex-1) {
								New_Total += parseInt($td1.text());
								Missing_Total += parseInt($td2.text());
							} else if (index == lastRowIndex-1) {
								$td1.html(New_Total);
								$td2.html(Missing_Total);
							}
						});
						$('#reportReview,#badReportReview').show();
					}
				}
			});
